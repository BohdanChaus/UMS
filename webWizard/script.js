on("ready", function(){
    ...
    Settings.prototype = {
        init: function(display, internal, fringeFactor, count, enabled){
            ...
            var label = elem("<label>", { textContent: "Наскрізний трафік", title: "Коефіцієнт наскрізного трафіку"});
            this.fringeFactor = elem("<input>", {type: "number", min: .5, max: 100, step: .1, value: fringeFactor});
            label.append(this.fringeFactor);
            options.append(label);

            label = elem("<label>", { textContent: "Кількість", title: "Генерується на годину на кілометр"});
    ...
    var vehicleClasses = [
        new Settings("Автомобілі", "passenger", 5, 12, true),
        new Settings("Вантажівки", "truck", 5, 8),
        new Settings("Автобуси", "bus", 5, 4),
        new Settings("Мотоцикли", "motorcycle", 2, 4),
        new Settings("Велосипеди", "bicycle", 2, 6),
        new Settings("Пішоходи", "pedestrian", 1, 10),
        new Settings("Трамваї", "tram", 20, 2),
        new Settings("Міські потяги", "rail_urban", 40, 2),
        new Settings("Потяги", "rail", 40, 2),
        new Settings("Судна", "ship", 40, 2)
    ];
    ...
    setPosition(25.327677, 50.747101);
